...
typedef enum perft_active_columns {
  PERFT_NODES = 0x01,
  PERFT_CAPTURES = 0x02,
...
perft_results_t position_5_results[5] = {{44, 0, 0, 0, 0, 0, 0, 0, 0},
...
const perft_results_t* results_tables[] = {
    default_board_results, Kiwipete_results, position_3_results,
    position_4_results, position_5_results};
...
void test_perft(perft_test_t test_case) {
  ...
  for (int depth = test_case.depth_of_first_row;
       depth <= test_case.depth_of_largest_row; depth++) {
    CAPTURE(depth);
    perft_results_t got = perft(&chess_state, depth, test_case.flags);
    perft_results_t expected = test_case.results_table[depth - test_case.depth_of_first_row];
    if (test_case.flags & PERFT_NODES) CHECK(got.nodes == expected.nodes);
    if (test_case.flags & PERFT_CAPTURES) CHECK(got.captures == expected.captures);
    if (test_case.flags & PERFT_ENPASSENT) CHECK(got.enpassent == expected.enpassent);
    if (test_case.flags & PERFT_CASTLES) CHECK(got.castles == expected.castles);
    if (test_case.flags & PERFT_PROMOTIONS) CHECK(got.promotions == expected.promotions);
    if (test_case.flags & PERFT_CHECKS) CHECK(got.checks == expected.checks);
    if (test_case.flags & PERFT_DISCOVER_CHECKS) CHECK(got.discovered_checks == expected.discovered_checks);
    if (test_case.flags & PERFT_DOUBLE_CHECKS) CHECK(got.double_checks == expected.double_checks);
    if (test_case.flags & PERFT_CHECKMATE) CHECK(got.checkmates == expected.checkmates);
  }
}

TEST_CASE("Perft Start Position", "[Perft]") {
